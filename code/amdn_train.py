...
def train(model, opt, dl_train, dl_val, logging, use_marks, 
          max_epochs, patience, display_step, save_freq, out_dir, device, args):
    ...
    for loop in range(2):
        ...
        for epoch in range(max_epochs):
            ...
            for input in dl_train:
                ...
                opt.zero_grad()
                loss = model(input.in_time.to(device), input.out_time.to(device),input.length.to(device), input.index.to(device), 
                             input.in_mark.to(device), input.out_mark.to(device), use_marks, device)
                '''if use_marks:
            ...
            if (epoch + 1) % save_freq == 0:
                if loop == 1:
                    ...
                    torch.save(best_model, os.path.join(out_dir, 'best_model_state_pre_dict_ep_{}.pt'.format(epoch)))
...
if __name__=='__main__':
    ...
    parser.add_argument('--no-wide', dest='wide', action='store_false')
    parser.set_defaults(wide=True)
    ...
    ## pre-set arguments
    parser.add_argument('--use_history', dest='use_history', action='store_true')
    parser.add_argument('--no-use_history', dest='use_history', action='store_false')
    parser.set_defaults(use_history=True)
    
    parser.add_argument('--use_marks', dest='use_marks', action='store_true')
    parser.add_argument('--no-use_marks', dest='use_marks', action='store_false')
    parser.set_defaults(use_marks=True)
    
    parser.add_argument('--use_embedding', dest='use_embedding', action='store_true', help='Seq embedding')
    parser.add_argument('--no-use_embedding', dest='use_embedding', action='store_false')
    ...
    gmm = GaussianLaplaceTiedMixture(2, 0, args.mark_embedding_size, device = args.device)
