...
def load_dataset(name, data_path=None, log_mode=True):
    """Load dataset."""
    if data_path:
        loader = pkl.load(open(data_path, 'rb'))
        cascades = loader['cascades']  # [0:1000]
        list_list_times = [list(cas[:, 1]) for cas in cascades]
        list_list_marks = [list(np.array(cas[:, 0], dtype=np.int32)) for cas in cascades]
        arrival_times = np.array(list_list_times)  # time scale is seconds for IRA dataset new-pkl obj
        # here the time of tweets is relative to source tweet in input.
        marks = np.array(list_list_marks); all_marks = []
        for list_marks in list_list_marks: all_marks += list_marks
        num_classes = np.max(all_marks) + 1 if marks is not None else 1 
    
    else:
        if not name.endswith(".pkl"):
            name += ".pkl"
        path_to_file = Path(dataset_dir) / name
        dataset = torch.load(str(path_to_file))
        arrival_times = [seq["arrival_times"] for seq in dataset['sequences']]
        marks = [seq.get('marks', None) for seq in dataset['sequences']]
        if marks[0] is None:
            marks = None
        start_times = [seq.get('start_time', 0) for seq in dataset['sequences']]
        arrival_times = [np.array(time) - start for time, start in zip(arrival_times, start_times)]
        num_classes = dataset.get('num_marks', 1)
    ...
    delta_times = [np.concatenate([[1.0], np.ediff1d(np.array(time))]) for time in arrival_times]
    # delta_times gives the inter-event times (time gap between current and previous post)
    for times in delta_times: times[times == 0] = 1e-3  # concurrent tweets cause inf in log transforms
    return SequenceDataset(delta_times, marks=marks, num_classes=num_classes, log_mode=log_mode)

def load_dataset_from_split(data, split, log_mode=True):
    ...
    arrival_times = data_split['arrival_times']
    marks = data_split.get('marks', None)
    delta_times = data_split['delta_times']  # first event at 1.0 and remaining concurrent events 1e-3 (log trans inf)
    num_classes = data['dim_process'] if marks is not None else 1
    return SequenceDataset(delta_times, marks=marks, num_classes=num_classes, log_mode=log_mode)
...
class SequenceDataset(data_utils.Dataset):
    ...
    def __init__(self, delta_times=None, marks=None, in_times=None, out_times=None,
        ...
        if self.in_marks is None:
            ...
            # All events are considered to be of type 0 and num_classes = 1
    ...
    def train_val_test_split_each(self, train_size=0.6, val_size=0.2, test_size=0.2, seed=123):
        ...
        data_train = SequenceDataset(in_times=in_train, out_times=out_train, in_marks=in_mark_train, 
                                     out_marks=out_mark_train, index=index_train, log_mode=False, 
    ...
    def flatten(self):
        ...
        flat_in_times = torch.cat(self.in_times)
        flat_out_times = torch.cat(self.out_times)
        return SequenceDataset(in_times=[flat_in_times], out_times=[flat_out_times], 
                               nlog_mode=False, num_classes=self.num_classes)
